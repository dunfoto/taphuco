...
const EditSocialComponent = React.memo(props => {
    ...
    const getSocial = async () => {
        try {
            const res = await axios.get(`/social/${id}`),
                { data: { data: { img, link } } } = res
            setValue('link', link)
            setImg(img)
            setOriginal(img)
            setValue('img', img)
    ...
    const onSubmit = async data => {
        try {
            const res = await axios.put(`/social/${id}`, data)
            if (res.status === 200) {
...
export default EditSocialComponent
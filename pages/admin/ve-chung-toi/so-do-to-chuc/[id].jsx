...
const NewSoDoToChucComponent = React.memo(props => {
    ...
    const getDetail = async () => {
        try {
            const res = await axios.get(`/board-director/${id}`)
            if (res.status === 200) {
                const { data: { data: { img, name, title, position } } } = res
                setValue('name', name)
    ...
    const onSubmit = async data => {
        try {
            ...
            const res = await axios.put(`/board-director/${id}`, data)
            if (res.status === 200) {

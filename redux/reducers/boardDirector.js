...
export const GET_BOARD_DIRECTORS = "GET_BOARD_DIRECTORS"
...
export const getBoardDirectors = () => async (dispatch, getState) => {
    try {
        const { boardDirector: { pagination: { page, limit } } } = getState(),
            res = await axios.get(`/board-directors?page=${page}&limit=${limit}`)
        dispatch({
            type: GET_BOARD_DIRECTORS,
            data: res.data.data,
            pagination: {
